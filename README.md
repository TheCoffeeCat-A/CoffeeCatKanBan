...
当前开发版本为 0.3.2, 在 M3 第一批任务操作和自适应看板布局基础上新增任务复制及单独确认的笔记删除, 保留安全创建, 列管理和属性撤销

这不是完成的三视图产品或整个 M3, 批量操作, 笔记关联, 完整列表, 月历及发布验收留在后续阶段, 原数据表保留用于验证
...
- 看板/数据表按钮切换显示方式, 原打开数据验证视图命令仍可使用, 页签 ID 保持兼容
...
当前工程通过 85 项测试: 54 项数据/仓库/查询测试, 17 项宿主控件替身测试, 5 项文件适配替身测试和 9 项架构测试, 26 个源码模块通过导入边界检查; 不替代真实 Obsidian 的文件系统和编辑器并发验收
...
- 复制/删除已有代码及受控测试, 真实宿主回收行为和外部竞争仍待专项验收; 批量处理, 自动关联/附件管理, 完整列表/月历, 全局设置页及发布验证尚未完成
